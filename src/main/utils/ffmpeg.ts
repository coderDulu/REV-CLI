...
export function transVideoToStream(url: any, stream: any) {
  try {
    const videoTxLocalProcess = ffmpeg(url)
      .on("start", (commandLine: any) => {
        // commandLine 是完整的ffmpeg命令
        console.log(commandLine, "转码 开始");
      })
      ...
      .addInputOption("-re", "-analyzeduration", "100000", "-max_delay", "1000000", "-stream_loop", "-1") // 这里可以添加一些 RTSP 优化的参数
      .on("end", function () {
        console.log("转码 结束!");
      })
      .addOutputOption(
